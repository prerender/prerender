...
PrerenderServer.prototype.onRequest = function(req, res) {
    ...
    this._pluginEvent("beforePhantomRequest", [req, res], function() {
        ...
        item.on("timeout", function() {
            ...
            _this.handleFinishedItem(req, res, "timed-out", 404);
        });

        item.on("response", function() {
            _this.handleFinishedItem(req, res, item.response.status, item.response.statusCode, item.response.documentHTML);
        });
    });
};

PrerenderServer.prototype.handleFinishedItem = function(req, res, status, statusCode, documentHTML) {
