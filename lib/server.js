...
server.connectToChrome = function() {
	return new Promise((resolve, reject) => {
		...
		let connect = () => {
			CDP.Version(Object.assign({}, this.devToolsOptions), (err, info) => {
				if (err) {
	...
	return new Promise((resolve, reject) => {
		...
		Promise.all([
			...
			Network.requestWillBeSent((params) => {
				...
				if (params.request.url === req.prerender.url) {
					req.prerender.initialRequestId = params.request.requestId;
				}

				if (params.redirectResponse && params.redirectResponse.url === req.prerender.url && !this.shouldFollowRedirects(req)) {
			...
			Network.responseReceived((params) => {
				...
				if (this.options.logRequests) util.log('-', req.prerender.numRequestsInFlight, params.response.url);

				if (params.response.requestId == req.prerender.initialRequestId) {

					//if the first request was redirected, the URLs wont match
					if (params.response.url == req.prerender.url || this.shouldFollowRedirects(req)) {
