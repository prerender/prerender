...
server.checkDownload = function(req, res) {
    ...
    }, this.options.pageDoneCheckTimeout || PAGE_DONE_CHECK_TIMEOUT);
};
...
server.onResourceRequested = function (requestData, request) {
    if ((/google-analytics.com/gi).test(requestData.url) ||
        ...
        (/www.woopra.com\/track/gi).test(request.url)) {
...
server.checkIfPageIsDoneLoading = function(req, res, force) {
    var timeSinceStart = new Date().getTime() - req.prerender.downloadStarted.getTime() 
      , maxTime = this.options.resourceDownloadTimeout || RESOURCE_DOWNLOAD_TIMEOUT
      ...
      , waitAfterLastRequest = this.options.waitAfterLastRequest || WAIT_AFTER_LAST_REQUEST
