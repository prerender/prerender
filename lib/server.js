...
server.init = function(options) {
	...
	this.options.usePuppeteer = this.options.usePuppeteer || USE_PUPPETEER;
	this.options.serverlessCluster = this.options.serverlessCluster|| SERVERLESS_CLUSTER;

	if (this.options.usePuppeteer) {
        console.log("USING PUPPETEER", this.options.usePuppeteer);
		this.browser = require('./browsers/chrome-puppeteer');
	} else {
        console.log("USING CHROME", this.options.usePuppeteer);
...
server.firePluginEvent = function(methodName, req, res) {
	return new Promise((resolve, reject) => {
		...
		next = () => {
			...
			if (method) {
				try {
					console.log("PLUGIN EVENT", layer.name, methodName, i);
