...
const PuppeteerChromeBrowserless = require('./browsers/chrome-puppeteer-browserless');
const PuppeteerChromeStandalone = require('./browsers/chrome-puppeteer-standalone');
...
const USE_BROWSERLESS = !!process.env.USE_BROWSERLESS;

const SERVERLESS_CLUSTER = process.env.SERVERLESS_CLUSTER || 'ws://localhost:3123';
...
server.init = function(options) {
	...
	this.options.useBrowserless = this.options.useBrowserless || USE_BROWSERLESS;
	this.options.serverlessCluster = this.options.serverlessCluster|| SERVERLESS_CLUSTER;

	if (this.options.useBrowserless) {
		util.log("using browserless chrome");
		this.browser = new PuppeteerChromeBrowserless(this.options.serverlessCluster);
	} else {
		util.log("using standalone chrome");
		this.browser = new PuppeteerChromeStandalone(this.options.chromeLocation);
	}
...
server.startPrerender = function() {
	return new Promise((resolve, reject) => {
		...
		.catch((err) => {
			util.log(err);
			util.log(`Failed to start and/or connect to ${this.browser.name}.`);
			this.killBrowser();
			reject();
		});
	});
...
server.isLocalBrowser = function() {
	return this.browser && this.browser.isLocalBrowser();
}
...
server.listenForBrowserClose = function() {
	...
	this.browser.onClose(() => {
		// never called in case of browserless mode
		this.isBrowserConnected = false;
...
server.onRequest = function(req, res) {
	...
	req.prerender = { ...this.options, ...util.getOptions(req) };
	// Do not rename reqId!
	...
	.then(() => {
		...
		if (err) util.log(`Handling request failed. Req: ${req.prerender.reqId} - ${req.prerender.url} Err:`, err);
...
server.firePluginEvent = function(methodName, req, res) {
	return new Promise((resolve, reject) => {
		...
		next = () => {
			...
			if (method) {
				try {
					// console.log("PLUGIN EVENT", methodName, layer.name);
