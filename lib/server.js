...
var JS_TIMEOUT = config.js_timeout || 10 * 1000;
...
server.createPhantom = function() {
    ...
    args.push({
        ...
        onExit: function() {
            _this.phantom = null;
            debug('phantom crashed, restarting...');
            logger.error('phantom crashed, restarting...');
...
server.onPhantomCreate = function(phantom) {
    debug('started phantom');
    logger.info('started phantom');
...
server.onRequest = function(req, res) {
    ...
    logger.info('getting', req.prerender.url);
...
server.checkIfJavascriptTimedOut = function(req, res) {
    ...
    if (timeout && req.prerender.lastJavascriptExecution && new Date().getTime() - req.prerender.lastJavascriptExecution.getTime() < 2000) {
        ...
        debug('Experiencing infinite javascript loop. Killing phantomjs...');
...
server._sendResponse = function(req, res, options) {
  ...
  logger.info('Done:', req.prerender.statusCode, 'in', ms + 'ms', 'for', req.prerender.url, length, 'bytes');
  ...
  if(options && options.abort) {
     ...
     //     _this.phantom = null;
     // } catch(e) {
     //     debug('Error killing phantomjs from javascript infinite loop:', e);
     // }
  }
};
