...
server.createPage = function(req, res) {
  ...
  if(!this.phantom) {
    setTimeout(function(){
      _this.waitCount++;
      logger.warn('Waiting for this.phantom ... ', _this.waitCount);

      if (_this.waitCount === 50) {
        _this.createPhantom();
      }

      if (_this.waitCount >= 100) {
        logger.error('this.phantom never transpired! Abandoning request: ', req.prerender.url);
        _this.waitCount = 0;
        return res.send(504, {abort: true});
      }
    ...
    logger.info('Utilizing phantom worker: %s waitCount: %d', this.phantom.id, _this.waitCount);
    _this.waitCount = 0;
...
server.onResourceRequested = function (requestData, request) {
    if ((/google-analytics.com/gi).test(requestData.url) ||
        ...
        (/chartbeat.com/gi).test(requestData.url) ||
        (/fonts.googleapis.com/gi).test(requestData.url) ||
        (/stats.g.doubleclick.net/gi).test(requestData.url) ||
        (/typekit.net/gi).test(requestData.url) ||
        (/segment.io/gi).test(requestData.url) ||
        (/stripe.com/gi).test(requestData.url) ||
        (/mc.yandex.ru/gi).test(requestData.url)){

        console.log('aborting: '.yellow, requestData.url);
        request.abort();
    }
};
...
server.onResourceRequestedCallback = function (req, res, response) {
  console.log('onResourceRequestedCallback: %s', response.url);
...
server.checkIfPageIsDoneLoading = function(req, res, force) {
    var timeElapsed = new Date().getTime() - req.prerender.downloadStarted.getTime();
    var timedOut = timeElapsed > (this.options.resourceDownloadTimeout || RESOURCE_DOWNLOAD_TIMEOUT)
    ...
    if(req.prerender.stage < 1 && (force || (req.prerender.pendingRequests <= 0 && timeSinceLastRequest > (this.options.waitAfterLastRequest || WAIT_AFTER_LAST_REQUEST)) || timedOut)) {
        ...
        logger.info('Done loading [%s] lastReq: %dms timeElapsed: %dms', req.prerender.url, timeSinceLastRequest, timeElapsed);
...
server.evaluateJavascriptOnPage = function(req, res) {
    ...
    logger.debug('Evaluating JS on page...');
    req.prerender.page.evaluate(this.javascriptToExecuteOnPage, function(obj) {
        ...
        if(!obj.shouldWaitForPrerenderReady || (obj.shouldWaitForPrerenderReady && obj.prerenderReady)) {
            ...
            logger.debug('Done evaluating page... %s ms HTML length: %s', Date.now() - start, obj.html.length);
...
server.onPageEvaluate = function(req, res) {
    ...
    if (!req.prerender.documentHTML) {
      logger.error('Missing req.prerender.documentHTML - returning %d'.red, req.prerender.statusCode || 404)
...
server._sendResponse = function(req, res, options) {
  ...
  res.setHeader('X-Powered-By', 'WebflowPrerender');
  res.setHeader('X-Prerender-Time', ms + 'ms');
  if (req.prerender.documentHTML) {
