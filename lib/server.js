...
const PAGE_LOAD_TIMEOUT = process.env.PAGE_LOAD_TIMEOUT || 20 * 1000; // TODO VCs for testing
...
server.init = function(options) {
	...
	if (this.options.useBrowserless) {
		...
		this.browser = new PuppeteerChromeBrowserless();
	} else {
        console.log("USING CHROME");
		this.browser = new PuppeteerChromeStandalone();
	}
...
server.onRequest = function(req, res) {
	...
	.then(() => {
		...
		console.log("EEEEEEEEEEEEEEEEEEEEEEEEEEEE", err);
		if (err) util.log(err);
...
server._send = function(req, res) {
	console.log("ORIGINAL STAUS CODE", req.prerender.statusCode);
