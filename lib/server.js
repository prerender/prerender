...
server.checkIfPageIsDoneLoading = function(req, res, force) {
    ...
    req.prerender.page.evaluate(function() { 
        ...
        var timeSinceStart = new Date().getTime() - req.prerender.downloadStarted.getTime() 
          , maxTime = req.prerender.resourceDownloadTimeout || _this.options.resourceDownloadTimeout || RESOURCE_DOWNLOAD_TIMEOUT
          ...
          , waitAfterLastRequest = req.prerender.waitAfterLastRequest ||_this.options.waitAfterLastRequest || WAIT_AFTER_LAST_REQUEST
        ...
        if(req.prerender.stage < 1 && (downloadCompleteNotification || force || downloadComplete || timedOut)) {
            ...
            if(req.prerender.statusCode && req.prerender.statusCode >= 300 && req.prerender.statusCode <= 399) {
                ...
                req.prerender.timeoutChecker = setInterval(
                    _.bind(_this.checkIfJavascriptTimedOut, _this, req, res)
                  , (_this.options.jsCheckTimeout || JS_CHECK_TIMEOUT)
                );
