...
server.onPhantomPageCreate = function(req, res) {
    ...
    req.prerender.page.set('onLoadStarted',function () {
        req.prerender.page.startTime = new Date();
    });
...
server.onResourceRequestedCallback = function (req, res, request) {
    var strt = new Date();
    req.prerender.resources[request.id] = {
        request: request,
        startReply: null,
        endReply: null,
        startTime: strt,
...
server.onResourceReceived = function (req, res, response) {
    if (response.stage === 'start') {
        req.prerender.resources[response.id].startReply = response;
    }
    if (response.stage === 'end') {
        req.prerender.resources[response.id].endReply = response;
    }
    ...
    if ('end' === response.stage) {
        ...
        req.prerender.resources[response.id].endTime = new Date();
    }
};
...
server._sendResponse = function(req, res, options) {
    ...
    req.prerender.endTime = new Date();
    var har = this._createHAR(req.prerender.url, "test", req.prerender.start, req.prerender.endTime, req.prerender.resources);
    var currentDate = new Date();
    var fileName = path.dirname(__dirname) + '/log/'+ currentDate.getDate() +'-'+currentDate.getMonth()+'-'+currentDate.getFullYear()+'/'+req.prerender.phantomId+'.har';
    console.log(req.prerender.phantomId);
    console.log(fileName);

    fs.outputJson(fileName, har, function(err) {
        if(err != null){
            console.log(err);
        }
    });
...
server._createHAR = function(address, title, startTime, endTime,resources)
{
    var _this = this;
    var entries = [];
    ...
    if( resources != null){
        resources.forEach(function (resource) {
            var request = resource.request,
                ...
                strtReqTime = resource.startTime,
                endReqTime = resource.endTime;
            ...
            entries.push({
                //To do find correct solution for wait &receive, sometimes startReply and endReply is null
                //startedDateTime: request.time.toISOString(),
                //time: endReply.time - request.time,
                startedDateTime: request.time,
                time: endReqTime - strtReqTime,
                request: {
                ...
                timings: {
                    ...
                    wait: endReqTime - strtReqTime,
                    receive: endReqTime - strtReqTime,
                    ssl: -1
                },
                pageref: address
            });
