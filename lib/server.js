var phantom = require('phantom')
  ...
  , resourceTranforms = require('./resources/resource-transforms.json');
...
var GZIP_LEVEL = zlib[process.env.PRERENDER_GZIP_LEVEL || 'Z_DEFAULT_COMPRESSION'];

var DOUBLE_ENDCODE_UNSAFE_URI = process.env.PRERENDER_DOUBLE_ENDCODE_UNSAFE_URI;
...
server.onPhantomPageCreate = function(req, res) {
    ...
    req.prerender.page.onResourceRequested(this.onResourceRequested, _.bind(_this.onResourceRequestedCallback, _this, req, res), _this.options.blockedResources || blockedResources, _this.options.resourceTranforms || resourceTranforms);
    ...
    req.prerender.page.get('settings.userAgent', function(userAgent) {
        userAgent = process.env.PRERENDER_USER_AGENT || (userAgent + ' Prerender (+https://github.com/prerender/prerender)');
        req.prerender.page.set('settings.userAgent', userAgent);
 ...
 */
server.onResourceRequested = function (requestData, request, blockedResources, resourceTranforms) {
    ...
    for(var j = 0,len = resourceTranforms.length, url, transform; j < len; j++) {
        transform = resourceTranforms[j];
        url = requestData.url.replace(new RegExp(transform.regexp, transform.flags), transform.sub);
...
server.onResourceReceived = function (req, res, response) {
    ...
    if(response.id === 1 && response.status >= 300 && response.status <= 399) {
        ...
        if (req.prerender._escaped_fragment_ !== undefined && req.prerender.redirectURL) {
            var delim = req.prerender.redirectURL.indexOf('?') >= 0 ? '&' : '?';
            req.prerender.redirectURL += delim + '_escaped_fragment_=' + req.prerender._escaped_fragment_;
        }
...
server.checkIfPageIsDoneLoading = function(req, res, force) {
    ...
    if(req.prerender.stage < 1 && (force || (req.prerender.status !== null && req.prerender.pendingRequests <= 0 && (timeSinceLastRequest > (req.prerender.waitAfterLastRequest || this.options.waitAfterLastRequest || WAIT_AFTER_LAST_REQUEST))) || timedOut)) {
        ...
        if(req.prerender.status === 'fail') {
            if (req.prerender.statusCode !== 404) {
...
server.evaluateJavascriptOnPage = function(req, res) {
    ...
    req.prerender.page.evaluate(this.javascriptToExecuteOnPage, function(obj) {
        ...
        // wait for all requests to finish, or for the prerenderReady flag
        if((!obj.shouldWaitForPrerenderReady && req.prerender.pendingRequests === 0) ||
           (obj.shouldWaitForPrerenderReady && obj.prerenderReady)) {
...
server._send = function(req, res, statusCode, options) {
    ...
    this._pluginEvent("beforeSend", [req, res], function() {
        ...
        if (res.headersSent) {
            // Response already sent, but before returning check if worker iterations # warrants phantom restart
            if((++_this.options.worker.iteration >= (_this.options.iterations || NUM_ITERATIONS)) || (options && options.abort)) {
        ...
        if (req.headers['accept-encoding'] && req.headers['accept-encoding'].indexOf('gzip') >= 0) {
            if (gzipped) {
                ...
                gzip(req.prerender.documentHTML, {level: GZIP_LEVEL}, function(err, result) {
                    if (!err) {
                        res.setHeader('Content-Encoding', 'gzip');
            ...
            zlib.gunzip(req.prerender.documentHTML, function(err, result) {
                if (!err) {
...
server._killPhantomJS = function() {
    ...
    var _this = this;
    this._pluginEvent("beforeKillPhantomJS", [], function() {
        require('tree-kill')(_this.options.worker.process.pid, 'SIGTERM');
    });
