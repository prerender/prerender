    ...
    req.prerender.page.run(_this.options.blockedResources || blockedResources, !!_this.options.logRequests, function(blockedResources, logRequests, resolve, reject) {
        ...
        this.onResourceRequested = function(requestData, request) {
            ...
            if(!requestData.aborted) {
                ...
                if(logRequests) {
                    console.log(new Date().toISOString(), '+', _this.prerender.resourcesRequested.length - _this.prerender.resourcesReceived.length - _this.prerender.resourcesTimeout.length - _this.prerender.resourcesError.length, requestData.url);
                }
            }
        };
        ...
        this.onResourceReceived = function(response) {
            ...
            if ('end' === response.stage) {
                if(response.url) {
                    ...
                    if(logRequests) {
                        console.log(new Date().toISOString(), '-', _this.prerender.resourcesRequested.length - _this.prerender.resourcesReceived.length - _this.prerender.resourcesTimeout.length - _this.prerender.resourcesError.length, response.url);
                    }
                }
        ...
        this.onResourceError = function(resourceError) {
            if(resourceError.url) {
                _this.prerender.resourcesError.push(resourceError);
            }
        }
...
server.checkIfPageIsDoneLoading = function(req, res) {
    ...
    req.prerender.page.run(function(resolve) {
        ...
        req.prerender.pendingRequests = response.resourcesRequested.length - response.resourcesReceived.length - response.resourcesTimeout.length - response.resourcesError.length;
