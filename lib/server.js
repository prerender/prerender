...
server.onPhantomPageCreate = function(req, res) {
    ...
    req.prerender.page.get('settings.userAgent', function(userAgent) {
        ...
        _this._pluginEvent("onPhantomPageCreate", [_this.phantom, req, res], function() {
            ...
            req.prerender.downloadChecker = _this.checkDownload(req, res);
...
server.checkDownload = function(req, res) {
    var _this = this;
    return setTimeout(function() {
        _this.pageLoadingWrap(req, res, req.prerender.status === 'fail');
    }, req.prerender.pageDoneCheckTimeout || this.options.pageDoneCheckTimeout || PAGE_DONE_CHECK_TIMEOUT);
};
...
server.pageLoadingWrap = function(req, res, force) {
    var _this = this;
    if (!force && req.prerender.page != null) {
        req.prerender.page.evaluate(function() { 
            if (typeof window.prerenderReady == 'boolean' && window.prerenderReady)
                return true;
            if (typeof window.prerenderDownloadComplete == 'boolean')
                return window.prerenderDownloadComplete
            return false;
        }, function(downloadCompleteNotification) {
            _this.checkIfPageIsDoneLoading(req, res, force || downloadCompleteNotification);
        });
    } else {
        this.checkIfPageIsDoneLoading(req, res, force);
    }
};
// Called occasionally to check if a page is completely loaded
server.checkIfPageIsDoneLoading = function(req, res, force) {
    var timeSinceStart = new Date().getTime() - req.prerender.downloadStarted.getTime() 
      , maxTime = req.prerender.resourceDownloadTimeout || this.options.resourceDownloadTimeout || RESOURCE_DOWNLOAD_TIMEOUT
      , timedOut = timeSinceStart > maxTime
      , timeSinceLastRequest = new Date().getTime() - req.prerender.lastResourceReceived.getTime()
      , noMoreDownloads = req.prerender.status !== null && req.prerender.pendingRequests <= 0 
      , waitAfterLastRequest = req.prerender.waitAfterLastRequest || this.options.waitAfterLastRequest || WAIT_AFTER_LAST_REQUEST
      , doneWaiting = timeSinceLastRequest > waitAfterLastRequest
      , downloadComplete = noMoreDownloads && doneWaiting
    ...
    if(req.prerender.stage < 1 && (downloadComplete || force || timedOut)) {
        req.prerender.stage = 1;
        req.prerender.downloadChecker = null;

        if(req.prerender.status === 'fail' && !!req.prerender.statusCode) {
        ...
        if(req.prerender.statusCode && req.prerender.statusCode >= 300 && req.prerender.statusCode <= 399) {
            ...
            req.prerender.timeoutChecker = setInterval(
                _.bind(this.checkIfJavascriptTimedOut, this, req, res)
              , (req.prerender.jsCheckTimeout || this.options.jsCheckTimeout || JS_CHECK_TIMEOUT)
            );
            this.evaluateJavascriptOnPage(req, res);
        }
    } else if(req.prerender.stage < 1) {
        //If no done conditions are complete, but the stage is still < 1, check again soon.
        req.prerender.downloadChecker = this.checkDownload(req, res);
    }
};
