...
class PuppeteerChromeBrowserless extends PuppeteerChrome {
	...
	async createNewPage() {
		...
		try {
			const serverlessCluster = this.serverlessCluster;
			const browser = await puppeteer.connect({ browserWSEndpoint: serverlessCluster}); // sadly no timeout option for this
