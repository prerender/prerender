			...
			.then(() => {
				...
				var checkIfDone = () => {
					...
					this.checkIfPageIsDoneLoading(tab).then((doneLoading) => {
						...
						tab.prerender.statusCodeReason = 'Chrome connection closed during request';
						reject();
					});
				};
				...
				Page.navigate({
					...
					tab.prerender.statusCode = 504;
					tab.prerender.statusCodeReason = 'invalid URL sent to Chrome: ' + tab.prerender.url;
					finished = true;
					reject();
				});
			}).catch((err) => {
...
chrome.checkIfPageIsDoneLoading = function (tab) {
	return new Promise((resolve, reject) => {
		...
		tab.Runtime.evaluate({
			...
			tab.prerender.statusCode = 504;
			tab.prerender.statusCodeReason = 'unable to evaluate javascript on the page';
...
chrome.printToPDF = function (tab, options) {
	return new Promise((resolve, reject) => {
		...
		var parseTimeout = setTimeout(() => {
			...
			tab.prerender.statusCode = 504;
			tab.prerender.statusCodeReason = 'print pdf timed out for ' + tab.prerender.url;
