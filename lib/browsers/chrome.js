      ...
      .then(() => {
        ...
        const execTabHealthCheck = async () => {
          ...
          tab.Runtime.evaluate({
            ...
            clearTimeout(tabHealthTimout);
            setTimeout(execTabHealthCheck, 1000);
          }).catch((err) => {
            clearTimeout(tabHealthTimout);
        ...
        const rejectByTabHealth = () => {
            util.log(`tab healthcheck rejected in ${Date.now() - healthCheckStarted}ms tabHealthLastSuccessValue=${tabHealthLastSuccessValue} url=${url}`);
