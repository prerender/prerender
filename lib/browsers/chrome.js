	...
	return new Promise((resolve, reject) => {
		...
		connectToBrowser(this.webSocketDebuggerURL, this.options.browserDebuggingPort)
			.then((chromeBrowser) => {
				...
				tab.browser = browser;
				tab.callFunction = this.callFunction;
...
chrome.setUpEvents = async function (tab) {
	...
	const updateMainFrame = (frame) => {
		if (!frame) {
			return;
		}
		const isMainFrame = !frame.parentId;
		if (isMainFrame) {
			tab.prerender.mainFrame = frame;
		}
	}

	const getFrameTreeResult = await Page.getFrameTree();

	// set blank page frame to main frame first
	updateMainFrame(getFrameTreeResult.frameTree.frame);

	Runtime.executionContextCreated((event) => {
		if (   event.context
			&& event.context.auxData
			&& event.context.auxData.isDefault
			&& tab.prerender.mainFrame
			&& event.context.auxData.frameId == tab.prerender.mainFrame.id) {
			tab.prerender.defaultExecutionContext = event.context;
		}
	});

	Page.frameNavigated((event) => updateMainFrame(event.frame));
...
chrome.callFunction = async (tab, functionDeclaration) => {
	const functionDeclarationText = functionDeclaration.toString();
	let retryCount = 2;
	for(;;) {
		try {
			return await tab.Runtime.callFunctionOn({
				functionDeclaration: functionDeclarationText,
				executionContextId: tab.prerender.defaultExecutionContext.id,
				userGesture: true
			});
		} catch(error) {
			if (
...
chrome.parseHtmlFromPage = function (tab) {
	return new Promise((resolve, reject) => {
		...
		let tooLongMarker = null;

		const PARSE_TOO_LONG_TIMEOUT = 5000;
		var parseTooLong = setTimeout(() => {
			util.log('parse html took too long', tab.prerender.url);
			tab.prerender.parseHtmlTookTooLong = true;
			tooLongMarker = new Date() - PARSE_TOO_LONG_TIMEOUT;
		}, PARSE_TOO_LONG_TIMEOUT);
		...
		this.callFunction(tab, getHtmlFunction).then((resp) => {
			if (tooLongMarker) {
				util.log(`get html took too long ${new Date() - tooLongMarker} ms ${tab.prerender.url}`);
				tooLongMarker = null;
			};
			...
			return this.callFunction(tab, getDoctype);
		}).then((response) => {

			if (tooLongMarker) {
				util.log(`get doctype took too long ${new Date() - tooLongMarker} ms ${tab.prerender.url}`);
				tooLongMarker = null;
			};
