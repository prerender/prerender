...
chrome.connect = function () {
  return new Promise((resolve, reject) => {
    ...
    let connect = () => {
      CDP.Version({ port: this.options.browserDebuggingPort })
        .then((info) => {
          this.originalUserAgent = info['User-Agent'];
          this.webSocketDebuggerURL =
            info.webSocketDebuggerUrl || 'ws://localhost:' + this.options.browserDebuggingPort + '/devtools/browser';
...
chrome.setUpEvents = async function (tab) {
  ...
  const userAgent =
    ...
    `${this.originalUserAgent} Prerender (+https://github.com/commudle/prerender)`;
  ...
  Network.responseReceived((params) => {
    ...
    if (params.type === 'EventSource') {
      ...
      tab.prerender.lastRequestReceivedAt = new Date().getTime();
      if (tab.prerender.logRequests || this.options.logRequests)
        util.log('-', tab.prerender.numRequestsInFlight, tab.prerender.requests[params.requestId]);
      delete tab.prerender.requests[params.requestId];
    }

    if (params.response && params.response.status >= 500 && params.response.status < 600) {
      // 5XX
  ...
  Network.loadingFinished(({ requestId, timestamp, encodedDataLength }) => {
    ...
    if (request) {
      ...
      if (tab.prerender.logRequests || this.options.logRequests)
        util.log('-', tab.prerender.numRequestsInFlight, tab.prerender.requests[requestId]);
      ...
      .then(() => {
          ...
          this.checkIfPageIsDoneLoading(tab)
            .then((doneLoading) => {
              if (doneLoading && !finished) {
                ...
                if (
                  (tab.prerender.renderType === 'jpeg' || tab.prerender.renderType === 'png') &&
        ...
        if (!tab.prerender.skipCustomElementsForcePolyfill) {
          Page.addScriptToEvaluateOnNewDocument({
            source: 'if (window.customElements) customElements.forcePolyfill = true',
          });
        }
    ...
    tab.Runtime.evaluate({
      ...
      .then((result) => {
        ...
        const timeSpentAfterFirstPrerenderReady =
          (tab.prerender.firstPrerenderReadyTime && new Date().getTime() - tab.prerender.firstPrerenderReadyTime) || 0;

        resolve(
          (!shouldWaitForPrerenderReady && doneLoading) ||
            (shouldWaitForPrerenderReady &&
              prerenderReady &&
              (doneLoading || timeSpentAfterFirstPrerenderReady > prerenderReadyDelay))
        );
      })
      .catch((err) => {
...
const getHtmlWithShadowDomFunction = () => {
  ...
  const attrStringList = attributeNames.map(
    (attributeName) => `${attributeName}="${htmlNode.getAttribute(attributeName)}"`
  );
