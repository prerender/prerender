  ...
  return new Promise((resolve, reject) => {
    ...
    Page.enable()
      .then(() => {
        ...
        function startCheckingHealth(label = '') {
          const TAB_HEALTH_EXEC_TIMEOUT_MS = 1000;
          const TAB_HEALTH_FAIL_TIMEOUT_MS = 1000;

          const log = msg => util.log(`[${label}] ` + msg);

          log(`Start checking health for tab url=${url}`)

          let tabHealthFailTimout;
          let tabHealthLastSuccessValue = 0;
          let tabHealthFinished = false;
  
          const execTabHealthCheck = async () => {
            if (finished || tabHealthFinished) {
              return;
            }
  
            let started = Date.now();
            //log(`executing tab healthcheck url=${url}`);
  
            tab.Runtime.evaluate({
              expression: 'window.prerenderHealthCheck = window.prerenderHealthCheck ? window.prerenderHealthCheck + 1 : 1',
            }).then(result => {
              const count = result?.result?.value;
              //log(`tab healthcheck success in ${Date.now() - started}ms count=${count} url=${url}`);
              tabHealthLastSuccessValue = count;
              clearTimeout(tabHealthFailTimout);
              setTimeout(execTabHealthCheck, TAB_HEALTH_EXEC_TIMEOUT_MS);
            }).catch((err) => {
              clearTimeout(tabHealthFailTimout);
              ...
              log(`tab healthcheck failed in ${Date.now() - started}ms err=${err}, url=${url}`);
              rejectByTabHealth();
            })
  
            tabHealthFailTimout = setTimeout(() => {
              if (finished || tabHealthFinished) {
                return;
              }
  
              log(`tab healthcheck failed by timeout url=${url}`);
              rejectByTabHealth();
            }, TAB_HEALTH_FAIL_TIMEOUT_MS)
          };
  
          let healthCheckStarted = Date.now();
  
          const rejectByTabHealth = () => {
              // log-only rejection for debug purposes
              log(`tab healthcheck rejected after ${Date.now() - healthCheckStarted}ms tabHealthLastSuccessValue=${tabHealthLastSuccessValue} url=${url}`);
              tabHealthFinished = true;
  
              /* tab.prerender.statusCode = 504;
              tab.prerender.statusCodeReason = 'tab healthcheck failed';
              finished = true;
              reject(); */
          }
        ...
        Page.navigate({
          ...
          .then(() => {
            startCheckingHealth('afterNavigate');
            setTimeout(checkIfDone, pageDoneCheckInterval);
          })
          .catch(() => {
