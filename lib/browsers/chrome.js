...
chrome.killForRestart = function () {
	return this._kill();
};
...
chrome.connect = function () {
	return new Promise((resolve, reject) => {
		...
		let connect = () => {
			...
			CDP.Version({ port: 3123}).then((info) => {
	...
	Network.requestWillBeSent((params) => {
		...
		if (params.redirectResponse) {
			...
			if (tab.prerender.initialRequestId === params.requestId && !tab.prerender.followRedirects && !this.options.followRedirects) {
				util.log(`Initial request redirected from ${entry.url} with status code ${params.redirectResponse.status}`);
