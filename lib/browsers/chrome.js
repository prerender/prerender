...
chrome.checkIfPageIsDoneLoading = function(tab) {
	return new Promise((resolve, reject) => {
		...
		tab.Runtime.evaluate({
			...
			if (prerenderReady && shouldWaitForPrerenderReady && !this.firstPrerenderReadyTime) {
				this.firstPrerenderReadyTime = new Date().getTime();
			}
			...
			const timeSpentAfterFirstPrerenderReady = (this.firstPrerenderReadyTime && (new Date().getTime() - this.firstPrerenderReadyTime)) || 0;

			resolve(		(!shouldWaitForPrerenderReady && doneLoading)
							 || (shouldWaitForPrerenderReady && prerenderReady && (doneLoading || timeSpentAfterFirstPrerenderReady > 1000))
			);
		}).catch((err) => {
