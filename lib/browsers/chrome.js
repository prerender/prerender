...
chrome.openInitialTabs = function() {
	return new Promise((resolve, reject) => {
		chrome.openTab().then((tab) => {
			this.emptyTabs.push(tab);
...
chrome.getEmptyTab = function(options) {
	return new Promise((resolve, reject) => {

		if (this.emptyTabs.length == 0) {
			return this.openTab().then((newTab) => {
				resolve(newTab);
			});
		}

		let tab = this.emptyTabs.shift();

		Object.assign(tab.prerender, options);

		resolve(tab);

		this.openTab().then((newTab) => {
			this.emptyTabs.push(newTab);
		});
	});
};

chrome.openTab = function() {
	return new Promise((resolve, reject) => {
