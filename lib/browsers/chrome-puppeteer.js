...
const conn = () => puppeteer.connect({ browserWSEndpoint: 'ws://localhost:3123' }); // TODO VCs address

const getRequestId = (request) => request._requestId;
...
puppeteerChrome.setUpEvents = async function (tab) {
	...
	page.on('request', (request) => {
		const requestId = getRequestId(request); // TODO VCs private field
		const timestamp = new Date().getTime();
		// trace("EVENT REQUEST", requestId);
	...
	page.on('response', (response) => {
		...
		let entry = tab.prerender.pageLoadInfo.entries[requestId];
		if (entry) {
			// entry.responseParams = params; // TODO VCs
		}
...
puppeteerChrome.checkIfPageIsDoneLoading = function (tab) {
	return new Promise((resolve, reject) => {
		...
		return tab.evaluate(() => {
			...
			trace("checkIfPageDoneLoading RESULT", result);
			...
			// trace("checkIfPageDoneLoading tab.prerender", tab.prerender.numRequestsInFlight);
...
// TODO: VCs megnezni a a resultokat
