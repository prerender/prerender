...
const conn = (options) => puppeteer.connect({ browserWSEndpoint: 'ws://localhost:3123' }); // TODO VCs address

// const conn = (options) => puppeteer.connect({ browserWSEndpoint: options.serverlessCluster}); // TODO VCs address
const getRequestId = (request) => request._requestId; // TODO VCs private field
...
puppeteerChrome.spawn = async function (options) {
	...
	const browser = await conn(options);
...
puppeteerChrome.kill = function () {
	trace("KILL");
	if (this.browser) {
		this.browser.close(); // TODO VCs disconnect?
	}
};
...
puppeteerChrome.connect = async function () {
	...
	let timeout = setTimeout(() => {
		if (!connected) {
			throw new Error(); // TODO VCs
		}
	}, 20 * 1000);

	// VCs With spawn we are already connected
...
puppeteerChrome.openTab = async function (options) {
	...
	page.emulateNetworkConditions({upload: 10*1024, download: 10*1024, latency: 0});
...
puppeteerChrome.setUpEvents = async function (tab) {
	...
	// 	deviceScaleFactor: 0,
	// 	mobile: false
	// });  TODO VCs these come from connect
	page.setViewport({
		width,
		height,
	...
	page.on('request', (request) => {
		...
		if (request.redirectChain().length > 0) {
			...
			if (tab.prerender.initialRequestId === requestId && !tab.prerender.followRedirects && !this.options.followRedirects) {
				...
				// tab.prerender.content = params.redirectResponse.statusText; // TODO VCs

				// Page.stopLoading(); TODO VCs ?????????????/
				this.stopLoading(tab);
				trace("SHOULD CALL PAGE STOPLOADING")
			}
		}
	});
	...
	page.on('response', (response) => {
		...
		if (entry && response.ok()) { // Response ok(): for example redirects do not have a response
			response.buffer().then(
				(responseBuffer) => {
					const size = responseBuffer.length;
					entry.responseLength += size;
				}).catch((err) => {console.log("CICAMICE", err)});
	...
	page.on('console', (msg) => {
		if (tab.prerender.captureConsoleLog || this.options.captureConsoleLog) {
			...
			tab.prerender.pageLoadInfo.logEntries.push({
				...
				// lineNumber: message.line, // TODO VCS ???
...
puppeteerChrome.loadUrlThenWaitForPageLoadEvent =  function (tab, url) {
	return new Promise( (resolve, reject) => {
		...
		try {	
			...
			// Page.addScriptToEvaluateOnNewDocument({ source: 'ShadyCSS = {shimcssproperties: true}' }) // TODO VCs how?
...
puppeteerChrome.stopLoading = async function (page) {
	const client = await page.target().createCDPSession(); // TODO VCs maybe preserve the client
