...
class PuppeteerChrome {
	...
	async setUpEvents (tab) {
		...
		const onResponse = (response) => {
			...
			util.trace('response', requestId, response.request()._url);
		...
		const onRequestFailed = (request) => {
			const requestId = getRequestId(request);
			util.trace('requestfailed ' + requestId + " " + request._failureText, request._url);
			if (!tab.prerender.requests[requestId]) {
				addRequestEvent(requestId, 'requestfailed', request, request._url);
				return;
			}
