...
prerender.onPhantomCreate = function(phantom) {
    ...
    this.phantom.numRequestsBeforeRestart = 50;
};
...
prerender.onRequest = function(req, res) {
    ...
    req.prerender.globalTimeout = setTimeout(function() {
        //sometimes phantom becomes unresponsive. This is just a fix in the meantime to figure out why.
        if(!res.sent) {
            console.log('response not sent in 30 seconds, killing phantom in case of unresponsiveness.')
            res.send(503);
            _this.killPhantomJSProcess();
        }
    }, 30000);
...
prerender.send = function(req, res, statusCode, documentHTML) {
    ...
    if(req.prerender.globalTimeout) {
        clearTimeout(req.prerender.globalTimeout);
        req.prerender.globalTimeout = null;
    }
    ...
    this.pluginsBeforeSend(req, res, function() {
        ...
        if (req.prerender.page) {
            ...
            _this.phantom.numRequestsBeforeRestart--;

            if(_this.phantom.numRequestsBeforeRestart <= 0) {
                console.log('restarting phantomjs process to combat memory leaks')
                _this.killPhantomJSProcess();
            }
        }
...
prerender.killPhantomJSProcess = function() {
