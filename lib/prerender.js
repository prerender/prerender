...
prerender.createPhantom = function() {
    phantom.create('--load-images=false', { binary: require('phantomjs').path }, _.bind(this.onPhantomCreate, this));
};
...
prerender.send = function(req, statusCode, documentHTML) {
    ...
    this.writeHead(statusCode, {'Content-Type': 'text/html;charset=UTF-8'});
    if (documentHTML) this.write(documentHTML);
    this.end();
    if (req.prerender.page) req.prerender.page.close();
    console.log('got', statusCode, 'for', req.prerender.url, 'in', new Date().getTime() - req.prerender.start.getTime() + 'ms')
};
...
prerender.checkIfPageIsDoneLoading = function(req, res) {
    ...
    if (timeout) {
        ...
        if(req.prerender.documentHTML) {
            return _this.onPageEvaluate(req, res);
        } else {
            return res.send(503);
        }
    }
    ...
    if (noPendingRequests) {
        req.prerender.page.evaluate(this.javascriptToExecuteOnPage, function(obj){
            ...
            } else if (!obj.shouldWaitForPrerenderReady) {
                clearInterval(req.prerender.interval);
                _this.onPageEvaluate(req, res);
            }
        });
...
prerender.waitForReady = function() {
    if(this.waitInterval) return;
