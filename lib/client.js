...
PrerenderClient.prototype.onResourceReceived = function (page, context, response) {
    ...
    if(normalizeUrl(context.request.url) === normalizeUrl(response.url) && response.status >= 300 && response.status <= 399) {
        if (response.redirectURL) {
            context.response.redirectURL = normalizeUrl(response.redirectURL);
        } else {
            var match = _.findWhere(response.headers, {name: 'Location'})
            if (match) {
                context.response.redirectURL = normalizeUrl(match.value);
            }
        }
        context.response.redirectURL = url.resolve(context.request.url, context.response.redirectURL)
