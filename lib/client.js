...
var CHECKER_TIMEOUT = process.env.PRERENDER_CHECKER_TIMEOUT || 50;
...
var JS_TIMEOUT_CHECKER = process.env.PRERENDER_JS_TIMEOUT_CHECKER || 50;

// Time in milliseconds to evaluate javascript on the web page
var EVALUATE_JAVASCRIPT_CHECKER = process.env.PRERENDER_EVALUATE_JAVASCRIPT_CHECKER || 50;

// Time in milliseconds to wait after the last request comes in to begin checking the page
var WAIT_AFTER_LAST_REQUEST = process.env.PRERENDER_WAIT_AFTER_LAST_REQUEST || 500;
...
function PrerenderClient(phantom, options) {
...
PrerenderClient.prototype.onQueueItemReady = function(item) {
    ...
    var context = {
        ...
        // When javascript started, after page is downloaded
        javascriptStarted: null,
    ...
    this.phantom.ph.createPage(function(page) {
        ...
        page.get('settings.userAgent', function(userAgent) {
            page.set('settings.userAgent', userAgent + ' Prerender (+https://github.com/prerender/prerender)');
...
PrerenderClient.prototype.checkIfPageIsDoneLoading = function(page, context, force) {
    ...
    if(context.stage < 1 && (force || (context.pendingRequests <= 0 && timeSinceLastRequest > WAIT_AFTER_LAST_REQUEST) || timedOut)) {
        ...
        if(context.response.statusCode && context.response.statusCode >= 300 && context.response.statusCode <= 399) {
            ...
            context.javascriptStarted = new Date;
...
PrerenderClient.prototype.checkIfJavascriptTimedOut = function(page, context) {
    ...
    var timeout = new Date().getTime() - context.javascriptStarted.getTime() > JS_TIMEOUT;
...
PrerenderClient.prototype.onPageEvaluate = function(page, context) {
    ...
    if (!context.response.documentHTML) {
        ...
        context.documentHTML = context.response.documentHTML;
        context.getContent = _.bind(this._getContent, this, context);
        context.setContent = _.bind(this._setContent, this, context);

        _this._pluginEvent('postRender', [context], function() {
            _this._pluginEvent("afterPhantomRequest", [page, context], function() {
...
PrerenderClient.prototype._getContent = function(context, callback) {
    var content = context.response.documentHTML;
    if (this.options.gzip) {
        zlib.gzip(new Buffer(content, 'utf-8'), function (err, zipped) {
            if (err) {
                callback(content);
            } else {
                callback(zipped.toString('base64'));
            }
        });
    } else {
        callback(content);
...
PrerenderClient.prototype._setContent = function(context, content) {
    context.response.documentHTML = content;
};
