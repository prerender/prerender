...
function disconnectBrowserIfBrowserShouldBeRestarted(server) {
  ...
  if(server.browserRequestsInFlight > 0 && new Date().getTime() - server.lastRestart > BROWSER_FORCE_RESTART_PERIOD) {
...
module.exports = {
  connectingToBrowserStarted: (req, res, next) => {
    disconnectBrowserIfBrowserShouldBeRestarted(req.server);
    next();
  }
